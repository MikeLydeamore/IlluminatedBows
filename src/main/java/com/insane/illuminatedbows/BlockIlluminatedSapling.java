...
public class BlockIlluminatedSapling extends BlockSapling {
	...
	protected BlockIlluminatedSapling() {
		super();
		float f = 0.4F;
	...
	private void growTree(World par1World, int par2, int par3, int par4) {
		...
		if (space) { //Generate tree
			par1World.setBlock(par2,par3,par4, IlluminatedBows.illuminatedWood);
			for (int i=1; i<6; i++) {
				if (par1World.isAirBlock(par2, par3+i, par4)) {
				par1World.setBlock(par2, par3+i, par4, IlluminatedBows.illuminatedWood);
				}
			}
			par1World.setBlock(par2,par3+6,par4,IlluminatedBows.illuminatedLeaves); //Top Leaf
			for (int i=3; i<7; i=i+2) { //Thin leaf layers
				if (par1World.isAirBlock(par2+1, par3+i, par4)) {
					par1World.setBlock(par2+1,par3+i, par4, IlluminatedBows.illuminatedLeaves);
				} if (par1World.isAirBlock(par2-1, par3+i, par4)) {
					par1World.setBlock(par2-1,par3+i, par4, IlluminatedBows.illuminatedLeaves);
				} if (par1World.isAirBlock(par2, par3+i, par4+1)) {
					par1World.setBlock(par2,par3+i, par4+1, IlluminatedBows.illuminatedLeaves);
				} if (par1World.isAirBlock(par2, par3+i, par4-1)) {
					par1World.setBlock(par2,par3+i, par4-1, IlluminatedBows.illuminatedLeaves);
				}
			}
			for (int i=2; i<6; i=i+2) {
				for (int z=-1; z<=1; z++) {
					if (par1World.isAirBlock(par2-2, par3+i, par4+z)) {
						par1World.setBlock(par2-2,par3+i,par4+z, IlluminatedBows.illuminatedLeaves);
					}
				}
				for (int z=-2; z<=2; z++) {
					if (par1World.isAirBlock(par2-1, par3+i, par4+z)) {
						par1World.setBlock(par2-1,par3+i,par4+z, IlluminatedBows.illuminatedLeaves);
					}
				}
				for (int z=-2; z<=2; z++) {
					if (z!=0) {
						if (par1World.isAirBlock(par2, par3+i, par4+z)) {
							par1World.setBlock(par2,par3+i,par4+z, IlluminatedBows.illuminatedLeaves);
						}
					}
				}
				for (int z=-2; z<=2; z++) {

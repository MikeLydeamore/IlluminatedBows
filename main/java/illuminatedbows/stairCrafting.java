...
public class stairCrafting implements IRecipe {
		...
		for (int row=0; row<3; row++) {
			for (int col=0; col<= row; col++) {
				if (invStacks[row][col]==null) {
					...
					if (invStacks[row][col].itemID!=IlluminatedBows.illuminatedPlanksID) {
						return false;
					}
			}
		}
	...
	public ItemStack getCraftingResult(InventoryCrafting inventorycrafting) {
		if (inventorycrafting.getStackInRowAndColumn(0,0).itemID==IlluminatedBows.illuminatedPlanksID) {
